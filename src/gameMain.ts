...
class GameMain{
    ...
    /**角色容器层 */
    private roleLayer:laya.display.Sprite;
    /**玩家主角 */
    private hero:Role;
    ...
    /**升级等级所需要的积分数量 */
    private levelUpScore:number = 50;

    /**统计错过多少架敌机，每错过十架扣一滴血 */
    public static miss:number = 0;
    ...
    private loop():void {
        ...
        //游戏碰撞逻辑检测
        //遍历所有飞机，更改飞机状态
        for (let i:number = this.roleLayer.numChildren - 1; i >=0; i--) {
            //获取第一个角色
    ...
    private createEnemy(index:number, hp:number, speed:number, num:number):void {
        for (let i:number = 0; i < num; i++) {
            //创建敌人，从对象池创建
            let enemy:Role = laya.utils.Pool.getItemByClass('role', Role);
            //初始化角色类型、血量、速度
