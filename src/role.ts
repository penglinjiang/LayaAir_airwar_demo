...
class Role extends laya.display.Sprite {
    /**角色类型：hero:玩家飞机，enemy:敌人飞机，bullet:子弹，ufo:道具 */
    public type:string;
    /**角色血量 */
    public hp:number = 0;
    /**角色速度 */
    ...
    /**角色的动画 */
    private roleAni:laya.display.Animation;
    /**当前动画动作 */
    ...
    public propType:number = 0;//默认为飞机，后面注意修改
    /**子弹级别，吃子弹道具后升级 */
    public bulletLevel:number = 0;
    /**同时射出子弹数量 ,最多四颗子弹同时*/
    public shootNum:number = 1;
    /**子弹偏移的位置 */
    private bulletPos:Array<any> = [[0], [-15, 15], [-30, 0, 30], [-45, -15, 15, 45]];

    constructor() {
        super();
        //实例化动画
        this.roleAni = new laya.display.Animation();
        //加载IDE编辑的动画文件
    ...
    public update():void {
        ...
        //主角边界检查
        if (this.type === 'hero') {
            if (GameMain.miss >= 5) {//错过的飞机超过5架，就扣一滴血，然后重新计算
            ...
            if (this.x < this.roleAni.width/2) {//判断是否左出
                this.x = this.roleAni.width/2;
            } else if (this.x > 720 - this.roleAni.width/2) {//判断是否右出
                this.x = 720 - this.roleAni.width/2;
            }

            if (this.y < this.roleAni.height/2) {//判断是否上出
    ...
    private onComplete():void {
        ...
        if (this.roleAni.width === 0) {
            //获得动画矩形边界
            let bounds:laya.maths.Rectangle = this.roleAni.getBounds();
            //角色宽高赋值
    ...
    public shoot():void {
        //获取当前时间
        let time:number = laya.utils.Browser.now();
        //如果当前时间大于下次射击时间
        if (time > this.shootTime) {
            //获得发射子弹的位置数组
            let pos:Array<any> = this.bulletPos[this.shootNum-1];
            for (let i:number = 0; i < pos.length; i++) {
                //更新下次射击时间
                this.shootTime = time + this.shootInterval;
                //从对象池里创建一颗子弹
                let bullet:Role = laya.utils.Pool.getItemByClass('role', Role);
                //初始化子弹信息，子弹阵营和发射者相同
    ...
    private lostProp():void {
        ...
        //从对象池里面创建一个道具
        let prop:Role = laya.utils.Pool.getItemByClass('role', Role);
